...
class MeikiOcrProvider(OcrProvider):
    ...
    def scan(self, image: Image.Image) -> Optional[List[Paragraph]]:
        ...
        try:
            ...
            # --- 2. Transform the library's output to MeikiKai's format ---
            return self._to_meikikai_paragraphs(ocr_results, img_width, img_height)
    ...
    def _to_normalized_bbox(self, bbox_pixels: list, img_width: int, img_height: int) -> BoundingBox:
        """converts an [x1, y1, x2, y2] pixel bbox to a normalized MeikiKai BoundingBox."""
    ...
    def _to_meikikai_paragraphs(self, ocr_results: list, img_width: int, img_height: int) -> List[Paragraph]:
        """converts the final meikiocr result list into MeikiKai's Paragraph format."""
